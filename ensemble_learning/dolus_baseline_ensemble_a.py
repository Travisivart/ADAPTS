...
def main():
    ...
    for p in predicts:
        if p >= 0.5:
            labels_predict.append(1.0)
        else:
            labels_predict.append(0.0)

    executionTime = (time.time() - startTime)

    print len(labels_predict)
    print "Prediction time = %.8f" % executionTime
    print "The accuracy is: " + str(metrics.accuracy_score(labels_test, labels_predict))
    ...
    plot_confusion_matrix(cnf_matrix, classes=CLASS_NAME, file_name='state_a_unnormalized.pdf')

    # Plot normalized confusion matrix
    plt.figure()
    plot_confusion_matrix(cnf_matrix, classes=CLASS_NAME, normalize=True, file_name='state_a_normalized.pdf')
    plt.show()
    print cm
    print "The f1_score is: " + str(metrics.f1_score(labels_test, labels_predict, average='macro'))
